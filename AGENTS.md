...
- Keep administrator authorization centralized for `lamyeunglung20@gmail.com` and `science_ai@fss.edu.hk`. Both emails receive matching dashboard, admin API, and Firestore data-management privileges and are excluded from rankings. `lamyeunglung20@gmail.com` is the super administrator and may delete any account; `science_ai@fss.edu.hk` must never be allowed to delete the super-administrator account.
