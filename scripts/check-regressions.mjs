...
const checks = [
  ...
  ['Start-study photo draft must persist, remain deletable, and open a full preview', files.dashboard.includes("'draftImages', 'studyStart'") && files.dashboard.includes('deleteSavedStartImage') && files.dashboard.includes('setStartImagePreviewOpen(true)') && files.dashboard.includes('start-photo-preview-modal') && files.firestoreRules.includes('match /users/{userId}/draftImages/{draftId}')],
  ...
  ['Avatar selection must open a movable and resizable crop-box step before saving', files.dashboard.includes('setAvatarCropSource(source)') && files.dashboard.includes('renderCroppedAvatar') && files.dashboard.includes('avatar-crop-selection') && files.dashboard.includes("'north-west'") && files.dashboard.includes("'south-east'") && files.styles.includes('.avatar-crop-selection') && files.styles.includes('.avatar-crop-backdrop')],
  ...
  ['Administrators must be able to add and remove reward options without deleted defaults returning', files.admin.includes('addReward') && files.admin.includes('admin-add-reward') && files.admin.includes('deleteReward') && files.admin.includes('admin-delete-reward') && files.appConfig.includes('MAX_REWARD_OPTIONS = 12') && files.appConfig.includes('seenIds') && files.firestoreRules.includes('rewards.size() <= 12') && files.firestoreRules.includes('configuredRewardMatches(11)')],
  ...
  ['Reward requests must wait for administrator approval before sticker deduction', files.dashboard.includes("status: 'pending', deducted: false") && !files.dashboard.includes('removedStickerCount: removedStickerCount + reward.stickerCost') && files.admin.includes('resolveRedemption') && files.admin.includes('批准並扣除') && files.firestoreRules.includes('request.resource.data.deducted == false') && files.firestoreRules.includes("'removedStickerCount', 'stickerBonusCount'")],
  ...
  ['Administrators must be able to add or subtract persistent total study time', files.admin.includes('adjustUserStudyTime') && files.admin.includes('admin-study-time-manager') && files.dashboard.includes('studyMinuteAdjustment') && files.dashboard.includes('ownTotalMinutes') && files.firestoreRules.includes("'studyMinuteAdjustment', 'removedStickerCount', 'stickerBonusCount'")],
];
